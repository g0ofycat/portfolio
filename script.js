document.addEventListener("DOMContentLoaded", () => {
  ...
  cards.forEach(card => {
    ...
    function rotateToMouse(e) {
      ...
      card.style.transform = `scale3d(1.07, 1.07, 1.07) rotate3d(${centerY / 100}, ${-centerX / 100}, 0, ${Math.log(distance) * 5}deg)`;
      card.style.transition = "transform 0.2s ease-out";

      if (glow) {
        glow.style.backgroundImage = `radial-gradient(circle at ${centerX * 2 + bounds.width / 2}px ${centerY * 2 + bounds.height / 2}px, rgba(255, 255, 255, 0.3), rgba(255, 255, 255, 0))`;
  ...
  function createTypewriterEffect(element, text, typingSpeed = 50, delay = 700) {
    ...
    cursor.style.cssText = "position:relative;display:inline-block;width:2px;height:1em;background-color:var(--primary);margin-left:2px;vertical-align:middle";
